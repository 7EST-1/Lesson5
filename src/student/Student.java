...
public class Student {
    ...
    public String validateData(){
        ...
        if (test1<0 || test1>100 || test2<0 || test2>100 || test3<0 || test3>100)
            if (em == null) //then there has been np error yet
                em = "At least 1 mark is out of the acceptable range";
        if (em != null) // then we have an error so add the follow line after it
            em += "\nPlease re-enter all the data\n";
                    return em; // return the error message, enther as null, or wth a reall message
